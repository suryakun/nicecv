      ...
      render={arrayHelpers => (
        <div className="">
          <ScrollArea className="h-fit flex flex-col gap-6">
            {props.form.values.education.map((exp, index) => (
              <div key={index} className="flex flex-col gap-4 mb-4">
                <div className="grid grid-cols-2 bg-accent rounded-sm items-center">
                  <div className="flex gap-2">
                    ...
                    <Button type="button" onClick={() => arrayHelpers.swap(index, index + 1)} disabled={index === props.form.values.education.length - 1} className="bg-transparent">
                      <ArrowDownIcon className="h-5 w-5 text-gray-500" />
                    </Button>
                  </div>
                  <h2 className="font-semibold text-primary float-right text-right pr-2">Education #{index + 1}</h2>
                </div>
                <Input type="text" placeholder="Name" name={`education[${index}].name`} onChange={props.form.handleChange} value={exp.name} />
                <Input type="text" placeholder="Education time" name={`education[${index}].time`} onChange={props.form.handleChange} value={exp.time} />
                <Textarea placeholder="Description" name={`education${index}].description`} onChange={props.form.handleChange} value={exp.description} />
                <Separator />
              </div>
            ))}
          </ScrollArea>
