...
export function PublicationInput<T extends FieldValues>(props: Props<T>) {
  const formName = 'publication' as ArrayPath<T>;
      ...
      <ScrollArea className="h-fit flex flex-col gap-6">
        {fields.map((exp, index) => (
          <div key={index} className="flex flex-col gap-4 mb-4">
            ...
            <CustomFormField
              control={props.control}
              name={`publication[${index}].name` as Path<T>}
              label="Name"
            />
            <CustomFormField
              control={props.control}
              name={`publication[${index}].time` as Path<T>}
              label="Publication time"
            />
            <CustomFormTextarea
              control={props.control}
              name={`publication[${index}].description` as Path<T>}
              label="Description"
            />
            <Separator />
          </div>
