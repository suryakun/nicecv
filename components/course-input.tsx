...
export function CourseInput<T extends FieldValues>(props: Props<T>) {
  const formName = 'course' as ArrayPath<T>;
      ...
      <ScrollArea className="h-fit flex flex-col gap-6">
        {fields.map((exp, index) => (
          <div key={index} className="flex flex-col gap-4 mb-4">
            ...
            <CustomFormField
              control={props.control}
              name={`course[${index}].name` as Path<T>}
              placeholder="Name"
              label="Name"
            />
            <CustomFormField
              control={props.control}
              name={`course[${index}].time` as Path<T>}
              placeholder="Time"
              label="Time"
            />
            <CustomFormTextarea
              control={props.control}
              name={`course[${index}].description` as Path<T>}
